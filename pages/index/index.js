...
const caculateDefault = require('../common/caculateDefault.js') // 引入计算的主体逻辑
  ...
  * case 3. 屏幕str最后1位为"."，则“:”覆盖之
  ...
  cal_promptly(str) {
    ...
    if (/^(?:(20|19)\d.(?:(?:0[1-9]|1[0-2])(?:0[1-9]|1[0-9]|2[0-8])|(?:0[13-9]|1[0-2])(?:29|30)|(?:0[13578]|1[02])31)|(?:[0-9]{2}(?:0[48]|[2468][048]|[13579][26])|(?:0[48]|[2468][048]|[13579][26])00)0229)$/g.test(str)){
      ...
      that.setData({ constellation_analysis: '触发彩蛋' })
...
let CaculateTime = {
  ...
  *@param    {bool}    time                   true-计算时间，false-计算数
  *@return    {array}  [equal_sign,res]       等于或者约等于，计算结果
  */
  , caculate_time: function (val, time) {
    val = val.replace(/\÷/g, '/');
    val = val.replace(/\×/g, 'x');
    ...
    if (reg.test(val)) {
      if (time) {
        val = this.dot_to_col(val);
      }
      else {
        val = this.col_to_dot(val);
      }
  ...
  */
  , two_actor: function (val) {
    let regcolcol = /^(-?)(\d+)(:)([0-5][0-9])([+-x/]-?)(\d+)(:)([0-5][0-9])$/g;
    ...
    if (is_match_regcolcol) {
      switch (sign_and_operator) {
        ...
        case "-,x-":
          ...
          M = (LH * 60 + LM) * (RH * 60 + RM);
          H = Math.floor(M / 3600);
          M = this.get_round((M % 3600) / 60, false);
      ...
      switch (sign_and_operator) {
        ...
        case "-,-":
        case "-,+-":
          RM = this.get_round((RM * 60) / 100, false);
          H = LH + RH;
          M = LM + RM;
      ...
      switch (sign_and_operator) {
        ...
        case "-,-":
        case "-,+-":
          LM = this.get_round((LM * 60) / 100, false);
          H = LH + RH;
          M = LM + RM;
  ...
  , dot_to_col(res) {
    ...
    if (reg.test(res)) {
      ...
      SM = RegExp.$4;
      M = ('' == SM) ? "00" : SM;
      M = (1 == SM.length) ? (SM + "0") : SM;
      M = Number(M);

      M = this.get_round(M * 60 / 100);
      M = (M > 9) ? M : ("0" + M);
